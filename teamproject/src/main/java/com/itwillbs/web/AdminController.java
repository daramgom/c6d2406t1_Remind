...
public class AdminController {
	...
	public ResponseEntity<String> checkUserEmail(@RequestBody String email) {

		logger.debug("selectMembers: " + email);

	    try {
	        MemberVO result = mService.memberEmailSearch(email);

	        if (result != null) {
	            // 중복된 이메일이 있을 경우
	            return ResponseEntity.ok("{\"result\": false, \"message\": \"중복된 이메일입니다.\"}");
	        } else {
	            // 사용 가능한 이메일인 경우
	            return ResponseEntity.ok("{\"result\": true, \"message\": \"사용 가능한 이메일입니다.\"}");
	        }
	    } catch (Exception e) {
	        // 예외 처리
	        logger.error("Error updating members", e);
